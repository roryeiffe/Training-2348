...
- Loosely Coupled - Making sure the components of our project aren't tightly connected. In other words, if one component changes in a loosely coupled application, then we wouldn't have to change too much for the rest of the project to still function. But in a tightly coupled application, one change could cause a lot of refactoring. 
- Maven helps us manage external dependencies, code that we didn't write, different packages, libraries. 
- Spring helps us manage the class/objects in our own application. 
